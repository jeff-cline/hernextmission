 ...
 * CTA mailto helpers.
 *
 * All call-to-action buttons across the site route to a mailto: link
 * pointing to krystalore@thecrewscoach.com. Each CTA carries a unique
 * subject (the button title + " - HER NEXT MISSION") and a body that
 * asks the right intake questions for that intent.
 *
 * Why mailto and not a form: while we're building toward a CRM, mailto
 * gives Krystalore a real-time inbox flow, full reply history, and zero
 * dependency on plugin state. Easy to upgrade to a form later — every
 * CTA is keyed by a stable slug.
 *
...
if (!defined('HNM_CTA_EMAIL')) {
    define('HNM_CTA_EMAIL', 'krystalore@thecrewscoach.com');
}

/**
 * Catalog of CTA intents. Keyed by short slug.
 *   - title: visible button text
 *   - intro: first paragraph of email body (frames intent)
 *   - questions: list of intake questions tailored to the intent
 *
 * Every CTA closes with a contact-info request, appended in
 * hnm_cta_body() so we don't repeat it everywhere.
 */
function hnm_cta_catalog(): array
{
    return [
        // --- Audience CTAs ----------------------------------------------------
        ...
        'sponsor-inquiry' => [
            ...
            'questions' => [
                ...
                'Sponsor tier you are exploring (Featured $25k · Lead $50k · Mission Partner $250k · custom):',
        ...
        'sponsor-deck' => [
            ...
            'questions' => [
                ...
                'Sponsor tier(s) of interest (Featured $25k · Lead $50k · Mission Partner $250k · custom):',
...
/**
 * Build the full email body for a given CTA slug.
 */
function hnm_cta_body(string $slug): string
{
    $catalog = hnm_cta_catalog();
    ...
    $lines[] = $cta['intro'];
    $lines[] = '';
    $lines[] = 'A few questions so we can come back ready:';
    $lines[] = '';
    foreach ($cta['questions'] as $q) {
        $lines[] = '• ' . $q;
        $lines[] = '';
    }
    $lines[] = '— Your contact info —';
    $lines[] = 'Name:';
    $lines[] = 'Best phone:';
    $lines[] = 'Best email:';
    $lines[] = 'City / time zone:';
    $lines[] = 'Best time to reach you:';
    $lines[] = '';
    $lines[] = 'Thank you. — Sent via hernextmission.org';

    return implode("\n", $lines);
}

/**
 * Build a complete mailto: URL for the given CTA slug.
 */
function hnm_cta_mailto(string $slug): string
{
    $catalog = hnm_cta_catalog();
    if (!isset($catalog[$slug])) {
        $slug = 'contact';
    ...
    $cta     = $catalog[$slug];
    $subject = $cta['title'] . ' - HER NEXT MISSION';
    $body    = hnm_cta_body($slug);
    ...
    return 'mailto:' . HNM_CTA_EMAIL . '?' . $query;
}

/**
 * Echo a complete mailto URL safely.
 */
function hnm_cta_href(string $slug): void
{
    echo esc_url(hnm_cta_mailto($slug));
}
